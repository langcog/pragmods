...
temp <- rrrsa::rsa.runDf(baserate_63,
                 quantityVarName = "object", semanticsVarName = "speaker.p",
                 itemVarName = "query", priorsVarName = "priorValue", alpha = 10, depth = 1)
cor(temp$p, temp$preds, use = "pairwise.complete.obs")
...
## Parallelize fitting
depths <- seq(0, 5) ## parallelize over depths
mclapply(depths, function(i) {
...
```{r}
## Base data frame of L0
sims_NoPrior <- plyr::ddply(fullData, .variables = c("grouper"), rsa.runDf,
            quantityVarName = "object", semanticsVarName = "speaker.p",
            itemVarName = "query", depth = 0) %>%
  rowwise %>%
  mutate(tuningData = ifelse(!is.na(count), "keep", "throwout"),
         depth = 0) %>%
  filter(tuningData == "keep")
...
for (L in Ls) {
  ...
  mutate(tuningData = ifelse(!is.na(count), "keep", "throwout"),
         depth = L) %>%
  filter(tuningData == "keep")
  sims_NoPrior <- rbind(sims_NoPrior, d)
}
```
...
## Parallelize fitting
depths <- seq(0, 5) ## parallelize over depths
noPriorFit <- mclapply(depths, function(i) {
  rsa.tuneDepthAlpha(fullData, quantityVarName = "object",
                               semanticsVarName = "speaker.p", itemVarName = "query",
                               groupName = "grouper",
                               compareDataName = "p", compareIndices = compareIndices, 
