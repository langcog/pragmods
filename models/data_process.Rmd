...
library(Deducer)
sims_prior %>%
  # select(cond, expt, matrix, object, preds, grouper, depth) %>%
  spread(depth, preds) %>%
  select("0", "1", "2", "3", "4", "5") %>%
  ggcorplot()
...
# corr.mat <- cor.matrix(variables = d(preds, as.factor(depth)),
#                        data = sims_prior,
#                        test = cor.test,
#                        method = "spearman",
#                        alternative = "two.sided", exact = FALSE)
# ggcorplot(corr.mat, data = sims_prior)
# 
# data(mtcars)
...
allData %>%
  ggplot(aes(x = preds, y = mdPreds, fill = expt, size = depth)) +
