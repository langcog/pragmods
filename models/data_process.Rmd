...
### Boostrap at depth 1
```{r}
## get depth 1 bstrap cis
d1 <- run_boot_sims(d_prior_counts, depth = 1, n_sims = 500, verbose = FALSE)

depth_sims <- mclapply(DEPTHS, FUN = function(depth) {
  run_boot_sims(d_prior_counts, depth = depth, n_sims = 100, verbose = FALSE)
  }, mc.cores = detectCores())
sims_prior_d1 <- sims_prior[which(sims_prior$depth == 1), ]
...
for (depth in seq(1, length(depth_sims))) {
  ...
  cis <- data.frame() ## Hold ci diffs
  ## For each simulation (row) in the current data frame
  for (sim_row in seq(1, nrow(curr_data))) {
    ## Get diff with our estimate
    diffs <- unlist(curr_data[sim_row, ]) - sims_prior[which(sims_prior$depth == depth - 1), ]$preds[sim_row]
    cis <- rbind(cis, diffs)
  }
  errs <- rbind(errs, data.frame(t(apply(cis, MARGIN = 1, FUN = quantile, c(0.05, 0.95)))))
}
# }

names(errs) <- c('low', 'hi')
sims_prior_bstrap_errs <- cbind(sims_prior, errs)
sims_prior_bstrap_errs <- sims_prior_bstrap_errs %>%
  mutate(ci_low = preds - low,
         ci_hi = preds - hi)


ggplot(sims_prior_bstrap_errs, aes(x = preds, y = p, col = expt, pch = object)) +
    geom_errorbarh(aes(xmin = ci_low, xmax = ci_hi, height = 0.05)) +
...
names(errs) <- c('hi', 'low')
sims_prior_d1 <- cbind(sims_prior_d1, errs)
sims_prior_d1 <- sims_prior_d1 %>%
  mutate(ci_low = preds - low,
         ci_hi = preds - hi)

ggplot(sims_prior_d1, aes(x = preds, y = p, col = expt, pch = object)) +
    geom_errorbarh(aes(xmin = ci_low, xmax = ci_hi, height = 0.05)) +
    geom_errorbar(aes(ymin = cil, ymax = cih, width = 0.05)) + 
    xlim(c(0,1)) + ylim(c(0,1)) + 
    ylab("Proportion choosing target") + 
...
cis <- t(as.data.frame(get_ci(d1)))
colnames(cis) <- c('bstrp_mu', 'bstrp_err')
sims_d1_bstrap_cis <- cbind(sims_prior[which(sims_prior$depth == 1), ], cis)

## Check how bstraps look... (appr normal???)
d1$simNum <- seq(1, nrow(d1))
d1_long <- d1 %>%
...
ggplot(sims_d1_bstrap_cis, aes(x = preds, y = p, col = expt, pch = object)) +
    geom_errorbarh(aes(xmin = bstrp_mu - bstrp_err, xmax = bstrp_mu + bstrp_err, height = 0.05)) +
    geom_errorbar(aes(ymin = cil, ymax = cih, width = 0.05)) + 
    xlim(c(0,1)) + ylim(c(0,1)) + 
    ylab("Proportion choosing target") + 
    xlab("Model Predictions") + 
...
colnames(cis) <- c('bstrp_mu', 'bstrp_err')
sims_prior_bstrap_cis <- cbind(sims_prior, cis)
```

### Look at boostrapped sims (appr normal?)
```{r}
boots_df <- convertListToDF(depth_sims[5])
...
### Temp analysis of bstrp trying to debug different means
```{r debugin_bstrap, eval = FALSE}
depth <- 2
