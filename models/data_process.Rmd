...
## Best model performance with fitted alphas
```{r}
...
depth_sims <- mclapply(DEPTHS, FUN = function(depth) {
  run_boot_sims(d_prior_counts, depth = depth, n_sims = 10, verbose = FALSE)
  }, mc.cores = detectCores())
# proc.time() - ptm
...
#          errorL = boot_se) %>%
  ggplot(., aes(x = preds, y = p, col = expt, pch = object)) +
    geom_errorbarh(aes(xmin = low, xmax = hi, height = 0.05)) +
