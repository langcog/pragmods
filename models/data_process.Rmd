...
#### Calc d5 bstrap cis
```{r calc_bstrap_cis_single_depth}
...
#### Calculate boostrapped CIs - all depths
```{r calc_bstrap_cis_all_depths}
...
ggplot(sims_prior_bstrap_errs, aes(x = preds, y = p, col = expt, pch = object)) +
  ...
  geom_smooth(method="lm", aes(group=1)) + 
  geom_abline(slope = 1, intercept = 0, lty=2) +
  ggtitle("Priors model, bootatrapped 95% CIs")
```
...
sims_prior_temp %>% filter(preds != 0, preds != 1) %>%
  ...
  geom_pointrange(aes(ymin = cil, ymax = cih)) + 
  xlim(c(0,1)) + ylim(c(0,1)) + 
  facet_wrap(expt~depth, labeller = label_both) +
  ylab("Proportion choosing target") + 
  xlab("Model Predictions") + 
  geom_smooth(method="lm", aes(group=1)) + 
  geom_abline(slope = 1, intercept = 0, lty=2) +
