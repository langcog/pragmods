...
sims_prior %>%
  ...
  ggplot(., aes(x = preds, y = p, col = expt, pch = object)) +
    ...
    ggtitle("Priors model sims, alpha = 1") + theme_bw()
```

### Recursive depth 0-5
```{r sims_priors_plot2, eval=FALSE}
...
ggplot(best_prior_models_fitted_alpha, aes(x = preds, y = p, col = expt, pch = object)) +
  ...
  geom_smooth(method="lm", aes(group=1)) + 
  geom_abline(slope = 1, intercept = 0, lty=2) +
  ggtitle("Model performance for recursive depth 0-5 with fitted alphas") + theme_bw()
```

# Simulations WITH 'Common knowledge priors' (recursive depth 0-5)

"Common knowledge" priors are only multiplied at the bottom level recursion.
...
## Correlations
```{r sims_ck_priors_cors, eval=FALSE}
sims_prior_ck %>%
  group_by(depth) %>%
  summarise("cor" = cor(p, preds)) %>%
  mutate("alpha" = 1)
